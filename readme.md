...
### 2-1. CafeMenuPanel 상세 구조

`MenuScreen`의 `[CENTER]` 영역입니다. 이 패널은 \*\*두 단계의 `CardLayout`\*\*을 중첩하여 사용합니다.

1.  **카테고리 전환:** `CardLayout` (예: "버거" 카드, "음료" 카드)
2.  **페이지 전환:** 각 "카테고리 카드" 내부에 또 다른 `CardLayout` (예: "버거 1/2 페이지", "버거 2/2 페이지")

  * **핵심 파일:** `CafeMenuPanel.java`
  * **루트 레이아웃:** `BorderLayout`
...
| |   | +------------------------------------------------+ |   | |
| |   | | [FlowLayout] [카테고리1] [카테고리2] [카테고리3] | |   | |
| |   | +------------------------------------------------+ |   | |
| +---+----------------------------------------------------+---+ |
+==================================================================+
| [CENTER] categoryContainer (CardLayout - 카테고리 전환)           |
|                                                                |
|  (현재 "카테고리1"이 선택된 상태)                              |
| +--------------------------------------------------------------+ |
...
### 1. 프로젝트 불러오기
1.  Eclipse를 실행합니다.
2.  `File` > `Open Projects from File System...`을 선택합니다.
3.  `Directory...` 버튼을 클릭하여 이 프로젝트의 루트 폴더(`KioskProject`)를 선택합니다.
4.  `Finish`를 클릭하여 프로젝트를 불러옵니다.
...
2.  파일을 마우스 오른쪽 버튼으로 클릭한 후 `Run As` > `Java Application`을 선택합니다.
